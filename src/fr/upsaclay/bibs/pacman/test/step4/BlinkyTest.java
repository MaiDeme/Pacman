...
public class BlinkyTest {
    ...
    @Test
    public void testSetGetElroy() throws PacManException {
        Board board = Board.createBoard(GameType.CLASSIC);
        board.initialize();
        board.startActors();
        Ghost blinky = board.getGhost(GhostType.BLINKY);
    ...
    @Test
    public void testElroyTarget() throws PacManException {
        Board board = Board.createBoard(GameType.CLASSIC);
        board.disableStateTime();
        board.initialize();
        board.startActors();
        ...
        blinky.setElroy(1);
        blinky.setGhostState(GhostState.CHASE);
        assertEquals(pacman.getCurrentTile(), blinky.getTarget());
        for(int i = 0; i < 10; i++) {
            board.nextFrame();
        }
        assertEquals(pacman.getCurrentTile(), blinky.getTarget());
        ...
        blinky.changeGhostState(GhostState.SCATTER);
        assertEquals(dir.reverse(), blinky.getIntention());
        assertEquals(pacman.getCurrentTile(), blinky.getTarget());
    }
}
